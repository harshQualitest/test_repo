...
/**
 * Props for {@link AnnotatorStats}.
 * @property projectBreakdowns - Per-project annotator statistics to aggregate and list.
 */
...
/**
 * Component: AnnotatorStats
 *
 * Purpose: Renders an annotator's performance statistics — a workspace-wide totals
 * strip (re-annotations, submitted, accepted, rejected) plus a per-project breakdown list.
 *
 * Responsibilities:
 * - Sum per-project counts into workspace-wide `totals` for the four stat buttons.
 ...
 * - `totals` is a client-side aggregation (sum) of each project's `reAnnotations`/`submitted`/`accepted`/`rejected`
 *   counts; this data currently comes from placeholder zeros (see `__need_dynamic_value_here__` markers in
 ...
 */
const AnnotatorStats = ({ projectBreakdowns }: AnnotatorStatsProps) => {
    ...
    const totals = projectBreakdowns.reduce(
        (acc, project) => ({
            reAnnotations: acc.reAnnotations + (project.reAnnotations || 0),
            submitted: acc.submitted + (project.submitted || 0),
            accepted: acc.accepted + (project.accepted || 0),
            rejected: acc.rejected + (project.rejected || 0),
        }),
        { reAnnotations: 0, submitted: 0, accepted: 0, rejected: 0 }
    );
            ...
            >
                ...
                >
                    ...
                    <Typography variant="h6" fontWeight={600} sx={{ color: theme.palette.warning.main }}>
                        {/* __need_dynamic_value_here__: Re-annotation count from API */}
                        {totals.reAnnotations || '__API__'}
                    </Typography>
                </Button>
                ...
                >
                    ...
                    <Typography variant="h6" fontWeight={600}>
                        {/* __need_dynamic_value_here__: Submitted count from API */}
                        {totals.submitted || '__API__'}
                    </Typography>
                </Button>
                        ...
                        >
                            ...
                            >
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                    <RefreshIcon sx={{ fontSize: 12, color: project.reAnnotations ? theme.palette.warning.main : 'text.secondary' }} />
                                    <Typography variant="caption" color="text.secondary">
                                        Re-Ann:
                                    </Typography>
                                    <Typography variant="caption" fontWeight={500} sx={{ color: project.reAnnotations ? theme.palette.warning.main : 'inherit' }}>
                                        {/* __need_dynamic_value_here__: Project-specific re-annotation count */}
                                        {project.reAnnotations || '__API__'}
                                    </Typography>
                                </Box>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                    ...
                                    <Typography variant="caption" fontWeight={500}>
                                        {/* __need_dynamic_value_here__: Project-specific submitted count */}
                                        {project.submitted || '__API__'}
                                    </Typography>
                                </Box>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
